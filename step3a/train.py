...
def train_one_epoch(model, train_loader, optimizer, base_lr, epoch, epochs, device, checkpoint_dir="/content/drive/MyDrive/checkpoints", start_batch=0):
    model.train()
    total_loss = 0.0
    ...
    for i, (images, labels) in enumerate(train_loader):
        ...
        try:
            ...
            for name, param in model.named_parameters():
                if param.grad is not None:
                    ...
                    if torch.isnan(grad_norm) or torch.isinf(grad_norm) or grad_norm > 1e3:
                        print(f" Anomalous gradient in '{name}' - norm: {grad_norm}")
            optimizer.step()
            ...
            print(f" RuntimeError at batch {i}: {e}")
            ...
            print(f"Crash saved at: {checkpoint_path}")
            continue
    ...
    print(f"Checkpoint epoch saved: {checkpoint_path}")
